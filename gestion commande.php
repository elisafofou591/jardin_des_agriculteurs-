...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['supprimer_commande'])) {
    ...
    if ($id_commande) {
        try {
            $stmt = $pdo->prepare(
                "DELETE FROM commandes WHERE id_commande = :id_commande"
            );
...
$statStmt = $pdo->query("
    SELECT
        COUNT(*) AS total,
        COALESCE(SUM(CASE WHEN statut = 'en_attente' THEN 1 ELSE 0 END), 0) AS attente,
        COALESCE(SUM(CASE WHEN statut = 'confirmee' THEN 1 ELSE 0 END), 0) AS validee,
        COALESCE(SUM(CASE WHEN statut = 'en_preparation' THEN 1 ELSE 0 END), 0) AS preparation,
        COALESCE(SUM(CASE WHEN statut = 'livree' THEN 1 ELSE 0 END), 0) AS livree,
        COALESCE(SUM(CASE WHEN statut = 'annulee' THEN 1 ELSE 0 END), 0) AS annulee,
        COALESCE(SUM(CASE WHEN statut <> 'annulee' THEN montant_total ELSE 0 END), 0) AS chiffre
    FROM commandes
");
...
.status.expediee{background:#e0f2fe;color:#0369a1}
                ...
                <table>
                    ...
                    <?php foreach ($commandes as $commande): ?>
                        <tr>
                            ...
                            <td>
                                <span class="status <?= e(classeStatut($commande['statut'] ?? 'en_attente')) ?>">
                                    <?= e(libelleStatut($commande['statut'] ?? 'en_attente')) ?>
                                </span>
                            </td>
